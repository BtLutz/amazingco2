...
class TreeNodeSerializer(serializers.ModelSerializer):
    ...
    root = serializers.PrimaryKeyRelatedField(queryset=TreeNode.objects.all(), required=False)
    ...
    def get_descendants(self, obj):
        ...
        return [TreeNodeDescendantSerializer(instance=d, context=self.context).data for d in descendants]

    def validate_root(self, data):
        if data:
            raise serializers.ValidationError('This field should be null for a TreeNode. We attach it ourselves.')
        return data
